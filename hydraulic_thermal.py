...
class Hydraulic_to_Thermal_P2_circulo(HydraulicThermal):
    
    # Problema 2: Cálculo do termo fonte (esquenta) e sumidouro (resfria) originado pela rede de microcanais.
    ...
    def solve_system_p2(self, S0, case_Ij, d_max=0.00025):
        ...
        if hasattr(self.thermal, 'get_index_inside_circus'):
            self.thermal.get_index_inside_circus()
            inside_circle = set(self.thermal.index_inside_circus)
        else:
            inside_circle = set()
        ...
        for i in range(Ny):
            for j in range(Nx):
                ...
                # Condições de Contorno das Bordas Externas da Placa e circulo
                if j == Nx - 1:
                    rows.append(Ic); cols.append(Ic); data.append(1.0); b[Ic] = TR
                elif j == 0:
                    rows.append(Ic); cols.append(Ic); data.append(1.0); b[Ic] = TL
                ...
                elif Ic in inside_circle:
                    rows.append(Ic); cols.append(Ic); data.append(1.0)
                    b[Ic] = self.thermal.circle_temp
